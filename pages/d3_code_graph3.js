...
data_days_counter = Array.apply(null, Array(7)).map(Number.prototype.valueOf,0);
  ...
  for (var i = 0; i < data.length; i++)
  {
   ...
   if (current_date.getDate() - data_date.getDate() <= 6 && data[i].frequency != '0')
   {
     ...
     data_days_counter[current_date.getDate() - data_date.getDate()] = 1; //don't average
...
data_days_counter = Array.apply(null, Array(31)).map(Number.prototype.valueOf,0);
  ...
  for (var i = 0; i < data.length; i++)
  {
   ...
   if (diffDays < 31)
    ...
    {
     data_days[data_date.getDate()] += parseInt(data[i].frequency);
     data_days_counter[data_date.getDate()] = 1; //we need to accumulate
    }
...
var tip = d3.tip()
  ...
  .offset([-10, 0])
  .html(function(d) {
    return "<strong>Calories burned:</strong> <span style='color:red'>" + d.frequency + "</span>";
  })

var placement = "#graph3";
...
var svg3 = d3.select(placement).append("svg")
