...
temp_data = [];
  ...
  t_start = new Date(t_s[0], t_s[1]-1, t_s[2] - 1, t_s[3], t_s[4], t_s[5]);
  t_end = new Date(t_e[0], t_e[1]-1, t_e[2], t_e[3], t_e[4], t_e[5]);

  //Go back one day
  //t_start = new Date(t_start - 2);
...
time_type = "Month"; //override - always month
...
var tip = d3.tip()
  ...
  .offset([-10, 0])
  .html(function(d) {
    return "<strong>Beats per minute:</strong> <span style='color:red'>" + d.frequency + "</span>";
  })

var placement = "#graph1";
